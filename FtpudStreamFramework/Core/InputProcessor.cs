...
namespace FtpudStreamFramework.Core
{
    public class InputProcessor
    {
        ...
        private void ParameterizedThreadStart(object source)
        {
            ...
            startInfo.FileName = StreamSettings.FfmpegConverter;
            startInfo.Arguments = sourceEntity.ProvideSourceCommandLine();
            
            LogUtils.Log(LogLevel.Debug, $"Starting process: {StreamSettings.FfmpegConverter}");
            LogUtils.Log(LogLevel.Debug, $"Args: {sourceEntity.ProvideSourceCommandLine()}");
